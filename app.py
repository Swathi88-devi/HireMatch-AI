# =========================================
# HireMatch-AI: Resume Fit Checker + Dashboard
# =========================================

# ----------------------------
# Install required packages (HF handles this automatically)
# ----------------------------
# pip install gradio pdfplumber python-docx nltk sentence-transformers torch plotly
...
def get_skills_score_semantic(resume_text, jd_text, similarity_threshold=0.7):
    ...
    # Semantic matching (limited for speed)
...
# ----------------------------
# Main Gradio analysis
# ----------------------------
def analyze_resume_and_jd(resume_file, resume_text_input, jd_text):
    try:
        resume_content = ""
        if resume_file is not None:
            ...
            return "Please upload or paste resume.", None, None, None, None, None, None, None
...
with gr.Blocks() as demo:
    gr.Markdown("## HireMatch-AI: Resume Fit Checker + Dashboard")

    with gr.Row():
        with gr.Column():
            resume_file = gr.File(label="Upload Resume (PDF/DOCX)")
            resume_text_input = gr.Textbox(lines=5, placeholder="Or paste resume text here...", label="Or paste resume text")
    ...
    analyze_button.click(
        ...
        inputs=[resume_file, resume_text_input, jd_text],
